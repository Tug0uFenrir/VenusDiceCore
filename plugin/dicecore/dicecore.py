...
def roll_single_dice(command):
    ...
    dice_str = dice_results
    return dice_str, total


def roll_dice(command):
    operator_part = re.findall(r'\d*d\d+|[+\-*/]|\d+', command)
    total = 0
    formula = ""
    current_op = "+"
    temp_value = None
    ...
    for item in operator_part:
        if re.match(r'\d*d\d+', item):
            ...
            temp_value = sub_total
            response.append(sub_result)
            formula += f' {current_op} {sub_result}'
        elif item.isdigit():
            temp_value = int(item)
            formula += f' {current_op} {item}'
        else:
...
def bot_info():
    text='''
    Dice筱酱By高老庄的土狗
    DiceCore:VenusCoreV2.0Bate
    CoreBy:高老庄的土狗
    GPTVersion:GPT-3.5-Turbo
    开源地址：https://github.com/Tug0uFenrir/VenusDiceCore.git
    '''
    return text
